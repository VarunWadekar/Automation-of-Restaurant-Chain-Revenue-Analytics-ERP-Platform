# BANGALURU UPLOADER - FINAL VERSION
Write-Host "Running SQL..." (Get-Date).ToString("HH:mm:ss")

sqlcmd -S $$$$ -d POSGoldQFile -U $$$$ -P '$$$$' `
-i "D:\VARUN\BANGALURU.sql" `
-o "D:\VARUN\FNB_ReportBANGALURU.csv" `
-s "|" -W -h-1 -w 65535
...
Import-Csv "D:\VARUN\FNB_ReportBANGALURU.csv" -Header $headers -Delimiter "|" | Where-Object { $_.BillNo -and $_.BillNo.Trim() -ne "" } | ForEach-Object {
...
Write-Host "BANGALURU upload completed." (Get-Date).ToString("HH:mm:ss")
Exit